...
t_hit		sub_inter_cone(t_object *obj, t_vect ray)
{
	...
	obj->dir = (t_3d_double){0,0,1};
	ray.ndir = v_norm(ray.dir);
	hit.is_hit = 0;
	inter.dist = v_sub_a_by_b(ray.pos, obj->pos);
	inter.norm = v_norm(obj->dir);
	inter.a = v_dot(ray.dir, ray.dir) - (1 + pow(tan(obj->radius), 2)) *
		pow(v_dot(ray.dir, inter.norm), 2);
	inter.b = 2 * (v_dot(ray.dir, inter.dist) - (1 + pow(tan(obj->radius), 2))
		* v_dot(ray.dir, inter.norm) * v_dot(inter.dist, inter.norm));
	inter.c = v_dot(inter.dist, inter.dist) - (1 +
		pow(tan(obj->radius), 2)) * pow(v_dot(inter.dist, inter.norm), 2);
	...
	inter.dist = v_sub_a_by_b(ray.pos, obj->pos);
	inter.temp = v_mult_by_nb(obj->dir, (v_dot(ray.dir, obj->dir) * hit.dist
	+ v_dot(inter.dist, obj->dir)));
	inter.temp = v_mult_by_nb(inter.temp, (1 + pow(tan(obj->radius), 2)));
	inter.temp2 = v_sub_a_by_b(hit.pos, obj->pos);
